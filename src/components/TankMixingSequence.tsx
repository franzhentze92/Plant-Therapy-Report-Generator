...
const TankMixingSequence: React.FC<TankMixingSequenceProps> = ({ selectedProducts, mixingItems, setMixingItems, onSummaryChange }) => {
  // Ensure mixingItems is always an array
  const safeMixingItems = Array.isArray(mixingItems) ? mixingItems : [];
  
  const addItem = () => {
    const newItem: MixingItem = {
      id: Date.now().toString(),
      sequence: safeMixingItems.length + 1,
      productDescription: '',
      products: [],
      notes: ''
    };
    setMixingItems([...safeMixingItems, newItem]);
  };

  const removeItem = (id: string) => {
    const newItems = safeMixingItems.filter(item => item.id !== id);
  ...
  const moveItem = (id: string, direction: 'up' | 'down') => {
    const currentIndex = safeMixingItems.findIndex(item => item.id === id);
    if ((direction === 'up' && currentIndex > 0) || (direction === 'down' && currentIndex < safeMixingItems.length - 1)) {
      const newItems = [...safeMixingItems];
  ...
  const updateItem = (id: string, field: keyof MixingItem, value: any) => {
    setMixingItems(items => {
      ...
      console.log('TankMixingSequence: setMixingItems called with:', updated);
      return updated;
    });
  };

  useEffect(() => {
    if (onSummaryChange) onSummaryChange(safeMixingItems);
  }, [JSON.stringify(safeMixingItems)]);
            ...
            <Card className="bg-white">
              <CardContent className="pt-4">
                <Table>
                  ...
                  <TableBody>
                    {safeMixingItems.map((item) => (
                      <TableRow key={item.id}>
                        <TableCell className="font-medium">{item.sequence}</TableCell>
                        <TableCell>
                          <Select
                            value={item.productDescription}
                            onValueChange={(value) => updateItem(item.id, 'productDescription', value)}
                          >
                            ...
                            <PopoverContent className="w-64">
                              <div className="flex flex-col gap-2">
                                {selectedProducts.map((product) => {
                                  ...
                                  const isSelectedElsewhere = safeMixingItems.some(
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            ...
                            <Button
                              ...
                              disabled={item.sequence === safeMixingItems.length}
                            >
