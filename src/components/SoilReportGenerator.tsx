...
// Mock data for demonstration (PLANT ANALYSIS PARAMETERS)
let mockNutrients = [
  { name: 'N - Nitrogen', current: 6, ideal: 4.0, unit: '%', category: 'plant' },
  { name: 'P - Phosphorus', current: 0.36, ideal: 0.35, unit: '%', category: 'plant' },
  { name: 'K - Potassium', current: 2.5, ideal: 2.5, unit: '%', category: 'plant' },
  { name: 'S - Sulphur', current: 0.18, ideal: 0.22, unit: '%', category: 'plant' },
  { name: 'Ca - Calcium', current: 1.5, ideal: 2.0, unit: '%', category: 'plant' },
  { name: 'Mg - Magnesium', current: 0.32, ideal: 0.40, unit: '%', category: 'plant' },
  { name: 'Na - Sodium', current: 0.05, ideal: 0.10, unit: '%', category: 'plant' },
  { name: 'Cu - Copper', current: 8.0, ideal: 10.0, unit: 'mg/kg', category: 'plant' },
  { name: 'Zn - Zinc', current: 25.0, ideal: 30.0, unit: 'mg/kg', category: 'plant' },
  { name: 'Mn - Manganese', current: 45.0, ideal: 50.0, unit: 'mg/kg', category: 'plant' },
  { name: 'Fe - Iron', current: 90.0, ideal: 100.0, unit: 'mg/kg', category: 'plant' },
  { name: 'B - Boron', current: 30.0, ideal: 35.0, unit: 'mg/kg', category: 'plant' },
  { name: 'Mo - Molybdenum', current: 0.88, ideal: 0.15, unit: 'mg/kg', category: 'plant' },
  { name: 'Si - Silicon', current: 120.0, ideal: 150.0, unit: 'mg/kg', category: 'plant' },
  { name: 'Co - Cobalt', current: 0.08, ideal: 0.10, unit: 'mg/kg', category: 'plant' },
].map(n => {
...
const SoilReportGenerator: React.FC<SoilReportGeneratorProps> = ({ paddockReports = [], setPaddockReports }) => {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (result.analyses && Array.isArray(result.analyses)) {
        ...
        setZoneSensitivity(prev => ({ ...prev, ...defaultSensitivity }));
        console.log('File upload complete, parsedNutrients:', filteredNutrients);
      }
    } catch (err) {
  ...
  const handleExportPDF = async () => {
    ...
    for (let i = 0; i < paddockReports.length; i++) {
      ...
      // Debug: print the data being exported for this paddock
      console.log('Exporting paddock', i, 'data:', data);
  ...
  const NutrientThresholdsPopup = ({ nutrients, colorThresholds, setColorThresholds, zoneSensitivity, setZoneSensitivity, onClose }) => {
    console.log('NutrientThresholdsPopup - nutrients:', nutrients);
    console.log('NutrientThresholdsPopup - colorThresholds:', colorThresholds);
    ...
    const [localThresholds, setLocalThresholds] = useState(() => {
      ...
      console.log('localThresholds initialized:', obj);
      return obj;
    });
    ...
    const handleSave = () => {
      console.log('Saving colorThresholds with localThresholds:', localThresholds);
      setColorThresholds(prev => {
        const updated = { ...prev };
        Object.entries(localThresholds).forEach(([n, vals]) => {
          updated[n] = typeof vals === 'object' && vals !== null ? { ...vals } : {};
        });
        console.log('Updated colorThresholds:', updated);
        return updated;
      });
  ...
  React.useEffect(() => {
    console.log('colorThresholds state updated:', colorThresholds);
  }, [colorThresholds]);
  ...
  // --- Comprehensive Nutrient Table Component ---
        ...
        // Debug: log the colorThresholds object
        console.log('ComprehensiveNutrientTable rendering with colorThresholds:', colorThresholds);
        console.log('zoneSensitivity:', zoneSensitivity);
    function getBarData(n) {
      ...
      if (n.range && low != null && high != null) {
        ...
        if (value > high) {
          ...
          // Debug log
          console.log(`Excessive logic for ${n.name}: value=${value}, high=${high}, deviationPercent=${deviationPercent}, absDeviation=${absDeviation}, threshold=${threshold}, green? ${absDeviation <= threshold}`);
          ...
          // Debug log
          console.log(`Deficient logic for ${n.name}: value=${value}, low=${low}, deviationPercent=${deviationPercent}, absDeviation=${absDeviation}, threshold=${threshold}, green? ${absDeviation <= threshold}`);
          if (absDeviation <= threshold) {
        ...
        if (value < low) bar = (value / low) * 0.333 * sensitivity.deficient;
          ...
          if (value <= low) {
            ...
            console.log(`${n.name} (fallback): value=${value}, deviation=${deviation.toFixed(1)}%, threshold=${threshold.toFixed(1)}, color=${color}, status=${status}`);
          } else if (value <= high) {
  ...
  const nutrientsToShow = React.useMemo(() => {
    ...
    if (!n || n.length === 0) return mockNutrients;
    // Filter out empty entries and duplicates
    ...
    return unique.length > 0 ? unique : mockNutrients;
  ...
  const updateCurrentPaddockData = (field, value) => {
    console.log('updateCurrentPaddockData:', field, value);
  ...
  console.log('DEBUG TankMix: currentTankMixProductNames:', currentTankMixProductNames);
  console.log('DEBUG TankMix: preFloweringFoliarProducts:', currentPaddockData.preFloweringFoliarProducts);
  console.log('DEBUG TankMix: nutritionalFoliarProducts:', currentPaddockData.nutritionalFoliarProducts);
      ...
      <div className="space-y-6 px-2 sm:px-4 lg:px-8">
            ...
            <CardContent>
              <div className="text-gray-800 text-base space-y-2">
                ...
                <ol className="list-decimal list-inside space-y-1">
                  ...
                  <li><strong>Save & export your report:</strong> Download or share a professional PDF report with all recommendations, charts, and supporting documents.</li>
                </ol>
              </div>
            </CardContent>
          </Card>
                      ...
                      <div className="flex gap-2 mb-6">
                        {paddockReports.map((report, idx) => (
                          <button
                            ...
                            type="button"
                          >
                            {availableAnalyses[idx]?.paddock && availableAnalyses[idx]?.paddock !== 'Unknown'
                              ? availableAnalyses[idx].paddock
                              : availableAnalyses[idx]?.name || `Paddock ${idx + 1}`}
        ...
        {uploadedFile && (
          <>
            ...
            <ReportSection title="7. Save & Export Report" collapsible expanded={showSection9} onToggle={() => setShowSection9(v => !v)} useHideButton={true} infoContent={"Save your work, download the report as a PDF, or share it. Use these options to finalize and distribute your soil analysis report."}>
              {showSection9 && (
                <Card className="bg-white mb-6">
                  <CardHeader>
                    ...
                    <div className="flex gap-4">
                      <Button onClick={() => generateCustomPDF(
                        paddockReports.map((paddock, idx) => {
                          ...
                          return {
                            ...rest,
                            paddockName: availableAnalyses[idx]?.paddock || availableAnalyses[idx]?.name || `Paddock ${idx + 1}`,
